...
@Component({
  selector: 'app-show-cmd',
  templateUrl: './show-cmd.component.html',
  styleUrls: ['./show-cmd.component.css'],
  animations: [
...
export class ShowCmdComponent implements OnInit,OnDestroy {
 ...
 selectAllb:Boolean=false;
  ...
  selectedCmdArray: Array<typecommade> =new Array<typecommade>();
...
//        }
  ...
  ngOnInit() {
    ...
    this.supp =false;
    let re = /\"/gi;
    this.moi = localStorage.getItem('username').replace(this.re,"");
    ...
    this.commandeService.getCommande(this.moi).subscribe((data:any) =>{
      ...
      //  this.dataSource = new MatTableDataSource(this.commande);
  ...
  selectedCmd(cmd,event){

    if (event.checked){
      this.selectedCmdArray.push(cmd);
      console.log(this.selectedCmdArray);
      }
      else{
      const index: number = this.selectedCmdArray.indexOf(cmd);
      if (index !== -1) {

        this.selectedCmdArray.splice(index, 1);
  ...
  selectAllFunction(event){
    if(event.checked){
      this.selectedCmdArray=  new Array<typecommade>();
      for(let i=0;i<this.commande.length;i++){
        this.selectedCmdArray.push(this.commande[i]);
      }
    }else {
      this.selectedCmdArray = new Array<typecommade>();
    }
...
if (this.moi == 'admin'){
  this.searchService.serchSecondDate(newDate1,newDate2)
    .subscribe((results:any) => {
  ...
  this.searchService.serchSecondDateUser(newDate1,newDate2,this.moi)
    .subscribe((results:any) => {
...
this.selection.clear();
  ...
  openDialog3(cmd) {
    ...
    this.dialog.open(Modifier, {
      data: {
        cmd : cmd,
        commande: this.dataSource2
    ...
    this.commandeService.getCommande(this.moi).subscribe((data:any) =>{
      ...
      if (!data.success) {
        ...
        this.dataSource2.data  = data.commande;
...
export class DialogDataExampleDialog {
  constructor(@Inject(MAT_DIALOG_DATA) public data: DialogData,private commandeService:CommandeService,private authService: AuthService,private adapter: DateAdapter<any>) {}
  ngOnInit(){
